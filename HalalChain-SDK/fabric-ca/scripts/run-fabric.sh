...
#   IFS=', ' read -r -a OORGS <<< "$ORDERER_ORGS"
   initOrdererVars ${ORDERER_ORGS[0]} ${ORDERER_ORG_DOMAINS[0]} ${CA_ORDERER_ORGS[0]} ${CA_ORDERER_ORG_DOMAINS[0]} ${ORDERER_MSP_IDS[0]} 1
...
#   IFS=', ' read -r -a PORGS <<< "$PEER_ORGS"
   ...
   for j in "${!PEER_ORGS[@]}"; do
      initPeerVars ${PEER_ORGS[j]} ${PEER_ORG_DOMAINS[j]} ${CA_PEER_ORGS[j]} ${CA_PEER_ORG_DOMAINS[j]} ${PEER_MSP_IDS[j]} $COUNT
   ...
   for k in "${!PEER_ORGS[@]}"; do
      initPeerVars ${PEER_ORGS[k]} ${PEER_ORG_DOMAINS[k]} ${CA_PEER_ORGS[k]} ${CA_PEER_ORG_DOMAINS[k]} ${PEER_MSP_IDS[k]} 1
      installChaincode
   done

   # Instantiate chaincode on the 1st peer of the 2nd org
   makePolicy
   initPeerVars ${PEER_ORGS[1]} ${PEER_ORG_DOMAINS[1]} ${CA_PEER_ORGS[1]} ${CA_PEER_ORG_DOMAINS[1]} ${PEER_MSP_IDS[1]} 1
   switchToAdminIdentity
   ...
   initPeerVars ${PEER_ORGS[0]} ${PEER_ORG_DOMAINS[0]} ${CA_PEER_ORGS[0]} ${CA_PEER_ORG_DOMAINS[0]} ${PEER_MSP_IDS[0]} 1
   ...
   initPeerVars ${PEER_ORGS[1]} ${PEER_ORG_DOMAINS[1]} ${CA_PEER_ORGS[1]} ${CA_PEER_ORG_DOMAINS[1]} ${PEER_MSP_IDS[1]} 2
   installChaincode
...
function makePolicy  {
   ...
   for i in "${!PEER_ORGS[@]}"; do
      ...
      POLICY="${POLICY}'${PEER_MSP_IDS[i]}.member'"
      COUNT=$((COUNT+1))
   done
   POLICY="${POLICY})"
   log "policy: $POLICY"
...
function createConfigUpdatePayloadWithCRL {
   logr "Creating config update payload with the generated CRL for the organization '$ORG_NAME'"
   ...
   cat config.json | jq '.channel_group.groups.Application.groups.'"${ORG_NAME}"'.values.MSP.value.config.revocation_list = ["'"${crl}"'"]' > updated_config.json
