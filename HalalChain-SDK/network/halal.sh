...
function printHelp () {
  echo "Usage: "
  echo "  halal.sh -m up|down|restart|generate [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>]"
  echo "  halal.sh -h|--help (print this message)"
  ...
  echo
  echo "	halal.sh -m generate -c mychannel"
  echo "	halal.sh -m up -c mychannel -s couchdb"
  echo "	halal.sh -m up -l node"
  echo "	halal.sh -m down -c mychannel"
  echo
  echo "Taking all defaults:"
  echo "	halal.sh -m generate"
  echo "	halal.sh -m up"
  echo "	halal.sh -m down"
}
...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/hlc.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/creater.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/transfer.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.hlc.com`` & ``peer0.creater.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  configtxgen -profile FourOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/hlcanchors.tx -channelID $CHANNEL_NAME -asOrg Hlc
  if [ "$?" -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for breed   ##########"
  echo "#################################################################"
  configtxgen -profile FourOrgsChannel -outputAnchorPeersUpdate \
  ./channel-artifacts/breedanchors.tx -channelID $CHANNEL_NAME -asOrg Creater
  if [ "$?" -ne 0 ]; then
    echo "Failed to generate anchor peer update for breed..."
    exit 1
  fi
  ...
  echo "#################################################################"
  configtxgen -profile FourOrgsChannel -outputAnchorPeersUpdate \
  ./channel-artifacts/butcheranchors.tx -channelID $CHANNEL_NAME -asOrg Transfer
  if [ "$?" -ne 0 ]; then
    echo "Failed to generate anchor peer update for breed..."
    exit 1
  fi
  ...
  echo "#################################################################"
  configtxgen -profile FourOrgsChannel -outputAnchorPeersUpdate \
  ./channel-artifacts/marketanchors.tx -channelID $CHANNEL_NAME -asOrg Seller
  if [ "$?" -ne 0 ]; then
    echo "Failed to generate anchor peer update for breed..."
    exit 1
  fi
