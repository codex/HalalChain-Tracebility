...
let upgradeChaincode = async function (peer, channelName, chaincodeName, chaincodeVersion, functionName, chaincodeType, args, username, org_name) {
    ...
    try {
        ...
        let results = await channel.sendUpgradeProposal(request, 120000); //upgrade takes much longer
            ...
            event_hubs.forEach((eh) => {
                let upgradeEventPromise = new Promise((resolve, reject) => {
                    ...
                    eh.registerTxEvent(deployId, (tx, code) => {
                        logger.info('The chaincode upgrade transaction has been committed on peer %s', eh._ep._endpoint.addr);
