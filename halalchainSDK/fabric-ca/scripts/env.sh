...
ORDERER_ORGS=(hlcorderer)
ORDERER_ORG_DOMAINS=(hlcorderer.com)
CA_ORDERER_ORGS=(hlc)
CA_ORDERER_ORG_DOMAINS=(hlc.com)
ORDERER_MSP_IDS=(hlcorderer)

# Names of the peer organizations
PEER_ORGS=(hlc creater transfer seller)
PEER_ORG_DOMAINS=(hlc.com creater.com transfer.com seller.com)
CA_PEER_ORGS=(hlc creater transfer seller)
CA_PEER_ORG_DOMAINS=(hlc.com creater.com transfer.com seller.com)
PEER_MSP_IDS=(hlc breed butcher market)
...
#
#ROOT_CA_ORGS=${ORDERER_ORGS}
#ROOT_CA_ORG_DOMAINS=${ORDERER_ORG_DOMAINS}
...
GENESIS_BLOCK_FILE=/$DATA/channel-artifacts/genesis.block
...
# initOrgVars <ORG_NAME> <ORG_DOMAIN>
function initOrgVars {
   if [ $# -ne 5 ]; then
      echo "Usage: initOrgVars <ORG_NAME> <ORG_DOMAIN> <CA_ORG_NAME> <CA_ORG_DOMAIN> <MSP_ID>"
      exit 1
   fi
   ORG_NAME=$1
   ORG_DOMAIN=$2
   ...
   CA_ORG_DOMAIN=$4
   MSP_ID=$5
   ORG_CONTAINER_NAME=${ORG_DOMAIN}
   ROOT_CA_HOST=rca.${ORDERER_ORG_DOMAINS[0]}
   ROOT_CA_NAME=rca-${ORDERER_ORGS[0]}
   ROOT_CA_LOGFILE=$LOGDIR/${ROOT_CA_NAME}.log
   INT_CA_HOST=ica.${CA_ORG_DOMAIN}
   INT_CA_NAME=ica-${CA_ORG_NAME}
   INT_CA_LOGFILE=$LOGDIR/${INT_CA_NAME}.log

   # Root CA admin identity
   ROOT_CA_ADMIN_USER=rca-${ORDERER_ORGS[0]}-admin
   ...
   ROOT_CA_INT_USER=ica-${ORDERER_ORGS[0]}-user
   ...
   INT_CA_ADMIN_USER=ica-${CA_ORG_NAME}-admin
   ...
   USER_NAME=${ORG_NAME}-user
   USER_PASS=${USER_NAME}pw

   ROOT_CA_CERTFILE=/${DATA}/${ORDERER_ORGS[0]}-ca-cert.pem
   INT_CA_CHAINFILE=/${DATA}/${CA_ORG_NAME}-ca-chain.pem
   ANCHOR_TX_FILE=/${DATA}/orgs/${ORG_DOMAIN}/anchors.tx
   ORG_MSP_ID=${MSP_ID}
   ORG_MSP_DIR=/${DATA}/orgs/${ORG_DOMAIN}/msp
   ORG_ADMIN_CERT=${ORG_MSP_DIR}/admincerts/cert.pem
   ORG_ADMIN_HOME=/${DATA}/orgs/${ORG_DOMAIN}/admin
...
function initOrdererVars {
   if [ $# -ne 6 ]; then
      echo "Usage: initOrdererVars <ORG_NAME> <ORG_DOMAIN> <CA_ORG_NAME> <CA_ORG_DOMAIN> <MSP_ID> <NUM>"
      exit 1
   fi
   initOrgVars $1 $2 $3 $4 $5
   NUM=$6
   ORDERER_HOST=orderer${NUM}.${ORG_DOMAIN}
   ORDERER_NAME=orderer${NUM}-${ORG_NAME}
...
# initPeerVars <ORG_NAME> <ORG_DOMAIN> <NUM>
function initPeerVars {
   if [ $# -ne 6 ]; then
      echo "Usage: initPeerVars <ORG_NAME> <ORG_DOMAIN> <CA_ORG_NAME> <CA_ORG_DOMAIN> <MSP_ID> <NUM>: $*"
      exit 1
   fi
   initOrgVars $1 $2 $3 $4 $5
   NUM=$6
   PEER_HOST=peer${NUM}.${ORG_DOMAIN}
   PEER_NAME=peer${NUM}-${ORG_NAME}
   ...
   if [ $NUM -gt 1 ]; then
      ...
      export CORE_PEER_GOSSIP_BOOTSTRAP=peer1.${ORG_DOMAIN}:7051
   fi
}
...
function switchToUserIdentity {
   export FABRIC_CA_CLIENT_HOME=/etc/hyperledger/fabric/orgs/$ORG_DOMAIN/user
   export CORE_PEER_MSPCONFIGPATH=$FABRIC_CA_CLIENT_HOME/msp
   if [ ! -d $FABRIC_CA_CLIENT_HOME ]; then
      dowait "$CA_NAME to start" 60 $CA_LOGFILE $CA_CHAINFILE
      log "Enrolling user for organization $ORG_NAME with home directory $FABRIC_CA_CLIENT_HOME ..."
      export FABRIC_CA_CLIENT_TLS_CERTFILES=$CA_CHAINFILE
...
function revokeFabricUser {
   ...
   export  FABRIC_CA_CLIENT_HOME=$ORG_ADMIN_HOME
   logr "Revoking the user '$USER_NAME' of the organization '$ORG_NAME' with Fabric CA Client home directory set to $FABRIC_CA_CLIENT_HOME ..."
   export FABRIC_CA_CLIENT_TLS_CERTFILES=$CA_CHAINFILE
...
function generateCRL {
   switchToAdminIdentity
   export FABRIC_CA_CLIENT_HOME=$ORG_ADMIN_HOME
   logr "Generating CRL for the organization '$ORG_NAME' with Fabric CA Client home directory set to $FABRIC_CA_CLIENT_HOME ..."
   export FABRIC_CA_CLIENT_TLS_CERTFILES=$CA_CHAINFILE
...
function copyAdminCert {
   ...
   if $ADMINCERTS; then
      dstDir=$1/admincerts
      mkdir -p $dstDir
      dowait "$ORG_NAME administator to enroll" 60 $SETUP_LOGFILE $ORG_ADMIN_CERT
